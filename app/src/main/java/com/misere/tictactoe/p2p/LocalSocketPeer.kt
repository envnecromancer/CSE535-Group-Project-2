...
class LocalSocketPeer(
    ...
    override suspend fun send(message: String) {
        try {
            ...
            _events.emit(PeerEvent.Error("Send error: ${e.message}"))
        }
    }
    ...
    override suspend fun stop() {
        ...
        _events.emit(PeerEvent.Disconnected("Connection stopped"))
    }
    ...
    override suspend fun startAdvertisingOrHosting(name: String) {
        withContext(Dispatchers.IO) {
            try {
                ...
                _events.emit(PeerEvent.Connected("Client joined"))
                setupStreams(socket!!)
                readLoop()
            } catch (e: Exception) {
            ...
            try {
                ...
                repeat(30) { attempt ->
                    try {
                        ...
                        _events.emit(PeerEvent.Connected("Connected on try ${attempt + 1}"))
                        return@withContext setupStreamsAndListen(sock!!)
                    } catch (e: Exception) {
                        delay(300)
                    }
                }
                _events.emit(PeerEvent.Error("Unable to connect to $hostIp:$port"))
            } catch (e: Exception) {
                _events.emit(PeerEvent.Error("Client setup error: ${e.message}"))
            }
        }
    }
    ...
    private fun readLoop() {
        scope.launch {
            try {
                ...
                _events.emit(PeerEvent.Disconnected("Read loop ended: ${e.message}"))
            }
        }
    }
}
